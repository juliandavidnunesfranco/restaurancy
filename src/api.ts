/* Definicion de Restaurante y sus tipos */
...
/* Funcion para esperar un tiempo antes de devolver la respuesta */
//const sleep = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));

/* Api con varias funciones que se pueden invocar en el proyecto */
const api = {

  //https://docs.google.com/spreadsheets/d/e/2PACX-1vR7vIiYQTtrfhE8MdiXQ6dmNSxStNYh9Dzoi5ENMFbWVccthlnoYmvfZuWQyW4FuDn1vGhtHfy9KXtx/pub?output=csv
  ...
  search: async (query?: string): Promise<Restaurant[] | string> => {
    if (!query) {
      // Búsqueda vacía, devolver todos los restaurantes
    ...
    const results = await api.list().then((restaurants) =>
      // Los filtramos por nombre (llevándolos a minúsculas)
    ...
    if (results.length === 0) {
      // No se encontraron resultados, devolver un mensaje
      return `No se encontraron restaurantes que coincidan con la búsqueda "${query}".`;
    }
    // Los retornamos
    return results;
  },
